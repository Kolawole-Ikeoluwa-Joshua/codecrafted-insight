...
# Brief description of prometheus components deployed:

```
1. Alertmanager: is responsible for handling alerts sent by Prometheus server
and manages their routing, grouping, and notification.

2. Blackbox Exporter: is used for probing endpoints over HTTP, HTTPS, DNS, TCP, and ICMP
to determine their health and status.

3. Grafana: is a popular open-source platform for monitoring and observability.
It allows you to create dashboards and visualize data from various data sources, including Prometheus.

4. Kube-state-metrics: is a service that listens to the Kubernetes API server and generates
metrics about the state of Kubernetes objects. It provides valuable information about the cluster's state.

5. Node Exporter: collects various metrics about the host machine and the operating system.

6. Prometheus Adapter: enables custom and dynamic queries against Prometheus metrics from Kubernetes.
It allows Kubernetes resources to be scaled based on custom metrics.

7. Prometheus: is a powerful open-source monitoring and alerting toolkit designed for reliability and scalability.
It collects, stores, and queries metrics from different services and systems.

8. Prometheus Operator: simplifies the deployment and management of Prometheus instances
and related monitoring components in a Kubernetes cluster.
```
